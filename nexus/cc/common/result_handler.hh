 ...
 *****************************************************************/
template <class C>
struct HeapResultHandler : ResultHandler<C> {
    using T  = typename C::T;
    using TI = typename C::TI;
    using ResultHandler<C>::threshold;

    T*  scores{nullptr};  // similarity
    TI* labels{nullptr};  // labels or id

    size_t topk;

    HeapResultHandler() = default;

    HeapResultHandler(size_t k, T* vals, TI* ids)
        : topk(k), scores(vals), labels(ids) {
        heapify();
    }

    void heapify() {
        heap_heapify<C>(topk, scores, labels);
        threshold = *scores;
        VLOG(1) << "Heapify for " << this << " threshold = " << threshold;
    }

    bool add_result(T score, TI label) {
        if (C::cmp(threshold, score)) {
            heap_replace_top<C>(topk, scores, labels, score, label);
            threshold = *scores;
            return true;
        }
        return false;
...
struct HeapBlockResultHandler : BlockResultHandler<C> {
    ...
    HeapBlockResultHandler(size_t nq, T* heap_dis_tab, TI* heap_ids_tab,
                           size_t k)
    ...
    struct SingleResultHandler : ResultHandler<C> {
        ...
        /// series of results for query i is done
        void end() { heap_reorder<C>(k, heap_dis, heap_ids); }
    };
...
//     void add(T val, TI id) { add_result(val, id); }
...
//     ReservoirBlockResultHandler(size_t nq, T* heap_dis_tab, TI* heap_ids_tab,
//                                 size_t k)
//         : BlockResultHandler<C>(nq),
//           heap_dis_tab(heap_dis_tab),
//           heap_ids_tab(heap_ids_tab),
...
//                 k, capacity, reservoir_dis.data() + (i - i0_2) * capacity,
...
//             const T*          dis_tab_i = dis_tab + (j1 - j0) * (i - i0) -
//             j0; for (size_t j = j0; j < j1; j++) {
//                 T dis = dis_tab_i[j];
...
//         // maybe parallel for
//         for (size_t i = i0; i < i1; i++) {
//             reservoirs[i - i0].to_result(heap_dis_tab + i * k,
//                                          heap_ids_tab + i * k);
//         }
//     }
// };
...
//         /// begin results for query # i
//         void begin(size_t i) { qr = &pres.new_result(i); }
